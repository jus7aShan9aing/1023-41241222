...
let isFlashing = false; // 用於標記是否閃光
const flashDuration = 0; // 閃光持續時間（毫秒）
let currentFlashTime = []; // 記錄每個磚塊的閃光狀態時間
    ...
    for (let c = 0; c < brickColumnCount; c++) {
        for (let r = 0; r < brickRowCount; r++) {
            ...
            if (b.hits > 0) {
                ...
                // 如果是閃光狀態，改變顏色
                if (isFlashing && flashDuration > 0) {
                    color = 'yellow'; // 閃光顏色
                    flashDuration--; // 減少閃光持續時間
                }
        ...
        for (let r = 0; r < brickRowCount; r++) {
            ...
            if (b.hits > 0) {
                if (x > b.x && x < b.x + brickWidth && y > b.y && y < b.y + brickHeight) {
                    console.log("Ball collided with brick:", b);
                    dy = -dy; // 垂直反彈
                    b.hits--; // 減少擊打次數
                    hitSound.currentTime = 0; // 重置音效時間
                    hitSound.play(); // 播放擊打音效
                    isFlashing = true; // 啟動閃光效果
                    flashDuration = 15; // 設置閃光持續時間（幀數）
                    if (b.hits === 0) {
                        score++; // 增加分數
                    }
                }
